  ...
  return <div className={`gridShell dataGrid-${id}`}><div className="gridTools"><div className="gridIdentity"><strong>{tableTitle}</strong>{description&&<small>{description}</small>}</div>{toolbarExtra}<span>{num(rows.length)} rows</span><button type="button" onClick={exportExcel}>Export Excel ↓</button><button type="button" onClick={e=>{e.stopPropagation();setChooser(v=>!v)}}>Columns ▾</button>{chooser&&<div className="columnChooser" role="group" aria-label="Visible table columns" onClick={e=>e.stopPropagation()}><strong>Show or hide columns</strong>{columns.map(c=><label key={c.key} onClick={e=>e.stopPropagation()}><input type="checkbox" checked={visible[c.key]!==false} onClick={e=>e.stopPropagation()} onChange={e=>setVisible(v=>({...v,[c.key]:e.target.checked}))}/><span>{c.label}</span></label>)}</div>}</div>
    <div className="tablePanel smartGrid"><table><thead><tr>{active.map(c=><th key={c.key} draggable onDragStart={e=>e.dataTransfer.setData("text/plain",c.key)} onDragOver={e=>e.preventDefault()} onDrop={e=>move(e.dataTransfer.getData("text/plain"),c.key)} onClick={()=>cycleSort(c.key)} className={`${c.numeric?"numeric":""} ${isChannelValue(c.label)?`channelColumnHead channel-${channelKey(c.label)}`:""}`}>{c.label}<i>{sort?.key===c.key?(sort.dir===1?" ↑":" ↓"):" ↔"}</i></th>)}</tr></thead>
      <tbody>{sorted.map((r,i)=>{const rowKey=String(r.id||r.rowId||r.name||r.date||r.week||i);const isExpanded=expanded===rowKey;return [
        <tr key={rowKey} onClick={()=>{if(renderExpanded)setExpanded(isExpanded?null:rowKey);onRowClick?.(r)}} className={`${(selectedKey&&(r.billNo===selectedKey||r.id===selectedKey))?"selected":""} ${renderExpanded?"expandableRow":""} ${(r.day==="Saturday"||r.day==="Sunday")?"weekendRow":""} ${rowClassName?.(r)||""}`}>{active.map((c,j)=>{const raw=c.value?c.value(r):r[c.key];return <td key={c.key} className={c.numeric?"numeric":""}>{j===0&&renderExpanded&&<span className="expandGlyph">{isExpanded?"−":"+"}</span>}{c.render?c.render(r):(isChannelColumn(c)&&isChannelValue(raw)?<ChannelBadge name={String(raw)}/>:String(raw??"—"))}</td>})}</tr>,
...
function MixDrill({invoices,paymentExceptions=[],metric="inclVat"}:{invoices:Invoice[];paymentExceptions?:any[];metric?:MixMetric}) {
  ...
  invoices.forEach(x=>{
    ...
    const row=matrix[x.orderType]??={name:x.orderType,orders:0,inclVat:0,vat:0,cells:{}};
    row.orders++;row.inclVat+=x.total;row.vat+=x.vat;
  ...
  return <div className="matrixDrill"><div className="matrixDrillHead"><div><h4>Order type × payment type</h4><p>Rows are supply channels; columns are tender allocations. Red cells require mapping review; split Card and Cash components are included separately.</p></div><Badge tone="good">{metric==="orders"?"ORDERS":metric==="vat"?"VAT":metric==="exVat"?"EXCL. VAT":"INCL. VAT"}</Badge></div><div className="matrixDrillScroll"><table><thead><tr><th>Order type</th>{tenders.map(t=><th className={`channelHead channel-${t.toLowerCase().replace(/\s+/g,"-")}`} key={t}>{t}</th>)}<th>Total</th><th>Dominant tender</th></tr></thead><tbody>{rows.map(r=>{const dominant=[...tenders].sort((a,b)=>value(r.cells[b]||{})-value(r.cells[a]||{}))[0];return <tr key={r.name}><td><ChannelBadge name={r.name}/><small>{num(r.orders)} invoices</small></td>{tenders.map(t=>{const amount=value(r.cells[t]||{}),bad=amount>0&&invalid(r.name,t);return <td className={bad?"matrixMismatch":""} key={t}>{amount?format(amount):"—"}{bad&&<small>Unexpected mapping</small>}</td>})}<td><strong>{format(value(r))}</strong></td><td><ChannelBadge name={dominant}/></td></tr>})}</tbody><tfoot><tr><td>TOTAL</td>{tenders.map(t=><td key={t}>{format(totals(t))}</td>)}<td>{format(totals())}</td><td>—</td></tr></tfoot></table></div></div>
}
...
export default function Home() {
  ...
  const zOrderRows=orderTypeNames.map(name=>{const fulfilled=data.invoices.filter(x=>x.status==="Fulfilled"&&x.orderType===name),cancelled=data.invoices.filter(x=>x.status!=="Fulfilled"&&x.orderType===name);const revenue=fulfilled.reduce((a,x)=>a+x.total,0),vat=fulfilled.reduce((a,x)=>a+x.vat,0),discount=fulfilled.reduce((a,x)=>a+x.discount,0),guests=fulfilled.reduce((a,x)=>a+Number(x.guests||0),0);return {name,orders:fulfilled.length,guests,revenue,discount,vat,net:revenue-vat,cancelled:cancelled.length,cancelledValue:cancelled.reduce((a,x)=>a+x.total,0),flag:hasChannelMismatch(fulfilled)}});
  const averageCards=[
  ...
  return <div className={`app ${sidebarCollapsed?"navCollapsed":""}`}>
    <aside className="sidebar">
      <div className="brand"><div className="logo">TMB</div><div><b>TMBill Audit</b><small>Revenue intelligence</small></div><button className="navToggle" onClick={()=>setSidebarCollapsed(v=>!v)} title={sidebarCollapsed?"Expand navigation":"Collapse navigation"}>{sidebarCollapsed?"›":"‹"}</button></div>
      <div className="period"><span>Review period</span><b>June 2026</b><small>The SS · Morbido Express</small></div>
      <nav>{tabs.map(([id,label],index)=><button key={id} title={label} className={tab===id?"active":""} onClick={()=>setTab(id)}><i>{String(index+1).padStart(2,"0")}</i><span>{label}</span>{id==="bills"&&<em>1</em>}{id==="items"&&<em>915</em>}</button>)}</nav>
    ...
    <main className="main">
      ...
      {tab==="overview"&&<>
        <section className="welcomeBanner"><div className="welcomeMark">TM</div><div><span>HELLO TMBILL TEAM</span><h2>Welcome to the TMBill Audit Dashboard</h2><p>Every reconciled day makes reporting clearer, operations stronger, and decisions safer. We are getting better—one controlled bill at a time.</p></div><button onClick={()=>setTab("guide")}>Start guided review →</button></section>
        ...
        <section className="averageSequence"><div className="sequenceHead"><div><span>MANAGEMENT AVERAGES</span><h3>June operating rhythm at a glance</h3></div><small>01–08 · arranged from time period to customer experience</small></div><div>{averageCards.map((x,i)=><button key={x.label} onClick={()=>setTab(x.tab)}><em>{String(i+1).padStart(2,"0")}</em><span>{x.label}</span><b>{x.value}</b><small>{x.sub}</small></button>)}</div></section>
      ...
      {tab==="zreport"&&<>
        ...
        <section className="zPreviewWorkspace">
          <div className="receipt80" id="management-z-receipt">
            <header><div className="receiptLogo">TMBILL</div><h2>MORBIDO EXPRESS RESTAURANT</h2><p>THE SS · MANAGEMENT Z REPORT</p><p>01 JUN 2026 12:45 PM – 30 JUN 2026 11:58 PM</p><b>CLOSED · RECONCILED</b></header>
            <div className="receiptRule">================================</div>
            <section><h3>SALES CONTROL</h3>{[
              ["Gross Item Sales",money(118066)],["Discounts",`(${money(10461.25)})`],["Complimentary",`(${money(13)})`],["Total reductions",`(${money(10474.25)})`],["Net Item Sales incl. VAT",money(107591.75)],["VAT included",money(5124.92)],["Net Item Sales excl. VAT",money(102466.83)],["Charges incl. VAT",money(581)]
            ].map(([a,b])=><div key={a}><span>{a}</span><b>{b}</b></div>)}<div className="receiptGrand"><span>ACTUAL SALES incl. VAT</span><b>{money(108172.75)}</b></div></section>
            <section><h3>BILL & CUSTOMER CONTROL</h3><div><span>Bill series</span><b>#2313–#4141</b></div><div><span>Fulfilled bills</span><b>1,816</b></div><div><span>Cancelled bills</span><b>13 · {money(1094.50)}</b></div><div><span>Recorded guests</span><b>1,817</b></div><div><span>Items sold</span><b>{num(+s.totalItemQuantity)}</b></div><div><span>Average cheque</span><b>{money(+s.averageCheck)}</b></div><div><span>Average per person</span><b>{money(+s.averagePerPerson)}</b></div><div><span>Avg items / bill</span><b>{(+s.totalItemQuantity/+s.fulfilledInvoices).toFixed(2)}</b></div></section>
            <section><h3>ORDER TYPE SUMMARY</h3>{zOrderRows.map(x=><article key={x.name} className={x.flag?"receiptFlagged":""}><h4>{x.name}{x.flag&&<em>POS FLAG</em>}</h4><div><span>Orders / guests</span><b>{num(x.orders)} / {num(x.guests)}</b></div><div><span>Revenue incl. VAT</span><b>{money(x.revenue)}</b></div><div><span>Discounts</span><b>{money(x.discount)}</b></div><div><span>VAT</span><b>{money(x.vat)}</b></div><div><span>Net excl. VAT</span><b>{money(x.net)}</b></div><div><span>Cancelled bills / value</span><b>{x.cancelled} / {money(x.cancelledValue)}</b></div>{x.flag&&<p>Operational flag: order type and tender mapping require TMBill POS review.</p>}</article>)}</section>
            <section><h3>PAYMENT SUMMARY</h3>{paymentTotals.map(([n,v,c])=><div key={n}><span>{n} · {num(c)} allocations</span><b>{money(v)}</b></div>)}<div className="receiptGrand"><span>Total collected</span><b>{money(108172.75)}</b></div><p className="receiptNote">Split tender allocations: 1,831 payment rows across 1,816 invoices.</p></section>
            <section><h3>DISCOUNT & CANCELLATION</h3><div><span>Order/promotional discounts</span><b>{money(10461.25)}</b></div><div><span>Complimentary</span><b>{money(13)}</b></div><div><span>Cancelled KOT lines</span><b>{num(kotAudit?.summary.cancelledItems||103)}</b></div><div><span>Cancelled KOT list value</span><b>{money(kotAudit?.summary.cancelledListValue||4794)}</b></div><div><span>Modified bills</span><b>18</b></div><div><span>Reprinted bills</span><b>402</b></div></section>
            <section><h3>OPERATING PERFORMANCE</h3><div><span>Average / median turnaround</span><b>{minutesLabel(businessInsights?.summary.overallAverageMinutes)} / {minutesLabel(businessInsights?.summary.overallMedianMinutes)}</b></div><div><span>P90 turnaround</span><b>{minutesLabel(businessInsights?.summary.overallP90Minutes)}</b></div><div><span>Average daily revenue</span><b>{money(+s.grossSales/30)}</b></div><div><span>Avg weekday / weekend</span><b>{money(weekPartRows[0].averageDaily)} / {money(weekPartRows[1].averageDaily)}</b></div><div><span>Best revenue date</span><b>Fri 12 Jun · {money(5405.90)}</b></div><div><span>Peak revenue hour</span><b>02 PM · {money(12518.10)}</b></div><div><span>Strongest week</span><b>22–28 Jun · {money(28685.65)}</b></div></section>
            <section className="receiptWarnings"><h3>TMBILL POS OPERATIONAL FLAGS</h3><p>• Dine In tendered as Talabat: {money(117)}</p><p>• Talabat orders tendered as Keeta: {money(65.60)}</p><p>• Tax-payment export omitted secondary split tenders: {money(1181.90)}</p><p>• Invoice charges {money(581)} vs DSR-classified charges {money(520)}: gap {money(61)}</p><p>• Charge VAT classification requires confirmation.</p></section>
        ...
        <section className="zSheet"><div className="zHead"><div><span>TAX / MANAGEMENT CONTROL REPORT</span><h2>Morbido Express Restaurant</h2><p>The SS · 01 Jun 2026 12:45 PM – 30 Jun 2026 11:58 PM</p></div><Badge tone="good">CLOSED · RECONCILED</Badge></div>
          ...
          <div className="zColumns"><section className="zBlock"><h3>Order types</h3>{paymentMatrix.map(x=><div key={x.orderType}><span><ChannelBadge name={x.orderType}/> ({x.orders})</span><b>{money(x.sales)}</b></div>)}<div className="grand"><span>Total</span><b>{money(108172.75)}</b></div></section>
