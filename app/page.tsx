...
function DataGrid({id,rows,columns,totals,onRowClick,selectedKey,renderExpanded,title,description,toolbarExtra}:{id:string;rows:any[];columns:GridCol[];totals?:Record<string,React.ReactNode>;onRowClick?:(r:any)=>void;selectedKey?:string;renderExpanded?:(r:any)=>React.ReactNode;title?:string;description?:string;toolbarExtra?:React.ReactNode}) {
...
function MixDrill({invoices,paymentExceptions=[],metric="inclVat"}:{invoices:Invoice[];paymentExceptions?:any[];metric?:MixMetric}) {
  ...
  return <div className="matrixDrill"><div className="matrixDrillHead"><div><h4>Order type × payment type</h4><p>Rows are supply channels; columns are tender allocations. Split Card and Cash components are included separately.</p></div><Badge tone="good">{metric==="orders"?"ORDERS":metric==="vat"?"VAT":metric==="exVat"?"EXCL. VAT":"INCL. VAT"}</Badge></div><div className="matrixDrillScroll"><table><thead><tr><th>Order type</th>{tenders.map(t=><th key={t}>{t}</th>)}<th>Total</th><th>Dominant tender</th></tr></thead><tbody>{rows.map(r=>{const dominant=[...tenders].sort((a,b)=>value(r.cells[b]||{})-value(r.cells[a]||{}))[0];return <tr key={r.name}><td><b>{r.name}</b><small>{num(r.orders)} invoices</small></td>{tenders.map(t=><td key={t}>{value(r.cells[t]||{})?format(value(r.cells[t])):"—"}</td>)}<td><strong>{format(value(r))}</strong></td><td><Badge tone="good">{dominant}</Badge></td></tr>})}</tbody><tfoot><tr><td>TOTAL</td>{tenders.map(t=><td key={t}>{format(totals(t))}</td>)}<td>{format(totals())}</td><td>—</td></tr></tfoot></table></div></div>
}
      ...
      {tab==="overview"&&<>
        ...
        {businessInsights&&<section className="twoCol insightCharts">
          ...
          <div className="panel" id="supply-collection-insights"><div className="panelHead"><div><h3>Supply mix × collection mix</h3><p>Sortable controls with fixed totals for two dimensions of {money(108172.75)}.</p></div><Badge tone="good">RECONCILED</Badge></div>
            <div className="mixTablesRow"><div><DataGrid title="Sales by order type" description="Supply channel performance" toolbarExtra={<MetricSelector value={mixMetric} onChange={setMixMetric}/>} id="overview-order-mix" rows={orderMixRows.map(x=>({...x,mix:x.value/Math.max(1,orderMetricTotal)*100}))} onRowClick={x=>{setOrderType(x.name);setTab("bills")}} columns={[
            ...
            <div><DataGrid title="Collections by payment mode" description="Tender allocations including split components" toolbarExtra={<MetricSelector value={mixMetric} onChange={setMixMetric}/>} id="overview-payment-mix" rows={paymentMixRows.map(x=>({...x,mix:x.value/Math.max(1,paymentMetricTotal)*100}))} onRowClick={()=>setTab("payments")} columns={[
        ...
        {businessInsights&&<>
          <section className="analyticsGrid">
            ...
            <div className="panel wide weekdayOrderPanel"><div className="panelHead"><div><h3>Order type × weekday sales mix</h3><p>Compare each supply channel by weekday. Dominant and lowest active order types are identified for faster scheduling and channel planning.</p></div><Badge tone="good">{money(108172.75)}</Badge></div>
              <DataGrid title="Order type × weekday sales mix" description="Click a weekday to expand order type × payment type" toolbarExtra={<MetricSelector value={mixMetric} onChange={setMixMetric}/>} id="weekday-order-mix" rows={weekdayOrderMix} renderExpanded={x=><MixDrill metric={mixMetric} paymentExceptions={paymentRecon?.exceptions} invoices={data.invoices.filter(i=>i.status==="Fulfilled"&&weekdayNames[invoiceDay(i).getDay()]===x.day)}/>} columns={[
          ...
          <section className="analyticsGrid">
            <div className="panel wide" id="daily-insights"><div className="panelHead"><div><h3>Daily revenue trend · June 2026</h3><p>Day numbers run from 1 to 30; hover a point for the exact revenue.</p></div><Badge tone="good">30 DAYS</Badge></div>
              ...
              <DataGrid title="Daily control table" description="Click a date to expand order type × payment type" toolbarExtra={<MetricSelector value={mixMetric} onChange={setMixMetric}/>} id="daily-performance" rows={businessInsights.daily.map(x=>({...x,displayValue:biMetric(x)}))} renderExpanded={x=><MixDrill metric={mixMetric} paymentExceptions={paymentRecon?.exceptions} invoices={invoicesForDay(x.date)}/>} columns={[
