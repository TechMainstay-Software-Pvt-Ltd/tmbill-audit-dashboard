...
export default function Home() {
  ...
  return <div className={`app ${sidebarCollapsed?"navCollapsed":""}`}>
    <aside className="sidebar">
      ...
      <nav>{tabs.map(([id,label])=><button key={id} title={label} data-label={label} className={tab===id?"active":""} onClick={()=>{setTab(id);if(sidebarCollapsed)setSidebarCollapsed(false)}}><i>{tabIcons[id]}</i><span>{label}</span>{id==="bills"&&<em>1</em>}{id==="items"&&<em>915</em>}</button>)}</nav>
        ...
        {businessInsights&&<>
          ...
          <section className="analyticsGrid">
            ...
            <div className="panel wide channelGroupPanel"><div className="panelHead"><div><h3>Own Orders × Aggregator Orders</h3><p>Restaurant-controlled channels compared with online food aggregators. Revenue is controlled; profitability awaits commission and food-cost inputs.</p></div><Badge tone="good">{money(+s.grossSales)}</Badge></div>
              <div className="channelGroupLayout"><div className="channelGroupCards">{channelGroupRows.map(x=><button key={x.name} onClick={()=>{setOrderType(x.name==="Own Orders"?"All":"Talabat");document.getElementById("overview-order-mix")?.scrollIntoView({behavior:"smooth"})}}><span>{x.name}</span><small>{x.description}</small><strong>{money(x.revenue)}</strong><i><em style={{width:`${x.mix}%`}}/></i><b>{x.mix.toFixed(1)}% of sales · {num(x.orders)} orders</b></button>)}</div><DataGrid title="Own × aggregator control" description="Click a row to expand order type × payment type" toolbarExtra={<MetricSelector value={mixMetric} onChange={setMixMetric}/>} id="channel-group-performance" rows={channelGroupRows} renderExpanded={x=><MixDrill metric={mixMetric} paymentExceptions={paymentRecon?.exceptions} invoices={data.invoices.filter(i=>i.status==="Fulfilled"&&x.types.includes(i.orderType))}/>} columns={[{key:"name",label:"Channel group"},{key:"orders",label:"Orders",numeric:true},{key:"guests",label:"Guests",numeric:true},{key:"displayValue",label:mixLabel,numeric:true,render:x=>mixValue(x.displayValue)},{key:"discount",label:"Discounts",numeric:true,render:x=>money(x.discount)},{key:"vat",label:"VAT",numeric:true,render:x=>money(x.vat)},{key:"averageCheck",label:"Avg check",numeric:true,render:x=>money(x.averageCheck)},{key:"mix",label:"Sales mix",numeric:true,render:x=>`${x.mix.toFixed(1)}%`}]} totals={{name:"TOTAL",orders:num(+s.fulfilledInvoices),guests:num(+s.totalGuests),displayValue:mixValue(channelGroupRows.reduce((a,x)=>a+x.displayValue,0)),discount:money(10461.25),vat:money(+s.vat),averageCheck:money(+s.averageCheck),mix:"100.0%"}}/></div>
            </div>
          </section>
      ...
      {tab==="zreport"&&<>
        ...
        <section className="zPreviewWorkspace">
          <div className="receipt80" id="management-z-receipt">
            ...
            <section><h3>ORDER TYPE SUMMARY</h3>{zOrderRows.map(x=><article key={x.name} className={x.flag?"receiptFlagged":""}><h4>{x.name}{x.flag&&<em>POS FLAG</em>}</h4><div><span>Orders / guests</span><b>{num(x.orders)} / {num(x.guests)}</b></div><div><span>Revenue incl. VAT</span><b>{printMoney(x.revenue)}</b></div><div><span>Discounts</span><b>{printMoney(x.discount)}</b></div><div><span>VAT</span><b>{printMoney(x.vat)}</b></div><div><span>Net excl. VAT</span><b>{printMoney(x.net)}</b></div><div><span>Cancelled bills / value</span><b>{x.cancelled} / {printMoney(x.cancelledValue)}</b></div>{x.flag&&<p>Operational flag: order type and tender mapping require TMBill POS review.</p>}</article>)}</section>
