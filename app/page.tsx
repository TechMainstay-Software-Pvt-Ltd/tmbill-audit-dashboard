      ...
      {tab==="zreport"&&<>
        ...
        <section className="zPreviewWorkspace">
          <div className="receipt80" id="management-z-receipt">
            ...
            <section><h3>ORDER TYPE SUMMARY</h3>{zOrderRows.map(x=><article key={x.name} className={x.flag?"receiptFlagged":""}><h4>{x.name}{x.flag&&<em>POS FLAG</em>}</h4><div><span>Orders / guests</span><b>{num(x.orders)} / {num(x.guests)}</b></div><div><span>Gross item sales</span><b>{printMoney(x.gross)}</b></div><div><span>Discounts / complimentary</span><b>{printMoney(x.discount)} / {x.complimentary?printMoney(x.complimentary):"—"}</b></div><div><span>Net item sales incl. VAT</span><b>{printMoney(x.netItemsInclVat)}</b></div><div><span>VAT</span><b>{printMoney(x.vat)}</b></div><div><span>Net item sales excl. VAT</span><b>{printMoney(x.net)}</b></div><div><span>Charges incl. VAT</span><b>{x.charges?printMoney(x.charges):"—"}</b></div><div><span>Actual sales incl. VAT</span><b>{printMoney(x.revenue)}</b></div><div><span>Cancelled bills / value</span><b>{x.cancelled||"—"} / {x.cancelledValue?printMoney(x.cancelledValue):"—"}</b></div>{x.flag&&<p>Operational flag: order type and tender mapping require TMBill POS review.</p>}</article>)}</section>
        ...
        <section className="zSheet"><div className="zHead"><div className="zTitleBrand"><div className="zLogoCrop"><img src="/tmbill-logo-source.png" alt="TMBill Technology LLC"/></div><div><span>TAX / MANAGEMENT CONTROL REPORT</span><h2>Morbido Express Restaurant</h2><p>The SS · 01 Jun 2026 12:45 PM – 30 Jun 2026 11:58 PM</p></div></div><Badge tone="good">CLOSED · RECONCILED</Badge></div>
          ...
          <div className="zColumns"><section className="zBlock"><h3>Order types · sales / charges</h3>{paymentMatrix.map(x=><div key={x.orderType}><span><ChannelBadge name={x.orderType}/> ({x.orders})</span><b>{money(x.sales)} · {x.charges?money(x.charges):"—"}</b></div>)}<div className="grand"><span>Total sales / charges</span><b>{money(108172.75)} · {money(581)}</b></div></section>
